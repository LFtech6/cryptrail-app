...
const Dashboard = ({ navigation }) => {
  ...
  const fetchConversionHistory = async () => {
    if (!user) return;
    try {
      const response = await fetch(`http://192.168.8.153:3000/getConversions/${user.id}`);
  ...
  useEffect(() => {
    const fetchInitialRates = async () => {
      setIsLoading(true);
      try {
        const response = await fetch("http://192.168.8.153:3000/rates");
  ...
  const convertCurrency = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.8.153:3000/convert?${query}`);
      const data = await response.json();
      ...
      if (data.convertedAmount) {
        ...
        await fetch('http://192.168.8.153:3000/saveConversion', {
          method: 'POST',
